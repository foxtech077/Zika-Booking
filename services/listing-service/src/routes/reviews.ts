...
export async function reviewRoutes(app: FastifyInstance) {
  ...
  app.post("/reviews", {
    ...
    return sendSuccess(reply, 201, { reviewId: review.id, message: "Review submitted successfully." });
      ...
      return sendError(reply, 500, "INTERNAL_ERROR", "An unexpected error occurred while submitting review.");
    }
  });
...
async function fetchProviderEmail(providerId: string): Promise<string | null> {
  ...
  app.get("/listings/:id/reviews", {
    ...
    return sendSuccess(reply, 200, {
      ...
      req.log.error({ err }, "Failed to fetch listing reviews");
  ...
  app.post("/reviews/:id/reply", {
    ...
    return sendSuccess(reply, 200, { message: "Reply submitted." });
      ...
      req.log.error({ err }, "Failed to submit provider reply to review");
      return sendError(reply, 500, "INTERNAL_ERROR", "An unexpected error occurred while submitting reply.");
    }
  });
  ...
  app.get("/reviews/me", {
    ...
    return sendSuccess(reply, 200, {
      ...
      req.log.error({ err }, "Failed to fetch guest's reviews");
      return sendError(reply, 500, "INTERNAL_ERROR", "An unexpected error occurred while fetching reviews.");
    }
  });
  ...
  app.patch("/reviews/:id/hide", {
    ...
    return sendSuccess(reply, 200, { message: `Review ${body.hidden ? "hidden" : "unhidden"} successfully.` });
      ...
      req.log.error({ err }, "Failed to hide/unhide review");
      return sendError(reply, 500, "INTERNAL_ERROR", "An unexpected error occurred while updating review visibility.");
    }
  });
}