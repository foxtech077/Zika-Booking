    ...
    if (line === "BEGIN:VEVENT") {
      ...
      if (current.dtstart && current.dtend) {
        events.push({
          ...
          uid:
            current.uid ??
            `${current.dtstart.toISOString()}-${current.dtend.toISOString()}-fallback`,
...
function parseIcalDate(value: string): Date {
  ...
  if (cleaned.length >= 15) {
    ...
    return new Date(
      `${year}-${month}-${day}T${hour}:${min}:${sec}${isUtc ? "Z" : ""}`,
    );
  }
...
export async function syncFeed(
  ...
  if (!normalizedUrl) {
    ...
    const backoffMs =
      failures === 1 ? 60_000 : failures === 2 ? 5 * 60_000 : 15 * 60_000;
...
export async function getDueIcalFeedIds(): Promise<string[]> {
...
export async function icalRoutes(app: FastifyInstance) {
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        return sendError(
          ...
          500,
          "INTERNAL_ERROR",
          "An unexpected error occurred while fetching iCal feeds.",
        );
      }
    },
  );
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!body.platform || !body.feedUrl) {
        return sendError(
          ...
          400,
          "VALIDATION_ERROR",
          "platform and feedUrl are required.",
        );
      }
      ...
      if (!normalizedUrl) {
        return sendError(
          ...
          400,
          "VALIDATION_ERROR",
          "feedUrl must be a valid http, https, or webcal URL.",
        );
      }
      ...
      try {
        ...
        const existing = await prisma.icalFeed.findFirst({
          where: { listingId: id, feedUrl: normalizedUrl },
        });
        if (existing) {
          return sendError(
            reply,
            409,
            "CONFLICT",
            "This feed URL is already connected to this listing.",
          );
        }
        ...
        return sendError(
          ...
          500,
          "INTERNAL_ERROR",
          "An unexpected error occurred while creating iCal feed.",
        );
      }
    },
  );
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        return sendError(
          ...
          500,
          "INTERNAL_ERROR",
          "An unexpected error occurred while deleting iCal feed.",
        );
      }
    },
  );
  ...
  app.post(
    "/listings/:id/ical-feeds/:feedId/sync",
    {
      preHandler: [requireUser],
      schema: {
        tags: ["iCal Calendar Sync"],
      ...
      try {
        ...
        if (result.error) {
          return sendSuccess(reply, 200, {
            synced: 0,
            error: result.error,
            message: "Sync failed — check the feed URL.",
          });
        }
        ...
        return sendError(
          ...
          500,
          "INTERNAL_ERROR",
          "An unexpected error occurred while syncing iCal feed.",
        );
      }
    },
  );
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
          ...
          from || to
            ? {
                OR: [
                  ...
                  {
                    ...(from
                      ? { pickupDatetime: { gte: new Date(from) } }
                      : {}),
        ...
        return sendError(
          ...
          500,
          "INTERNAL_ERROR",
          "An unexpected error occurred while fetching blocked dates.",
        );
      }
    },
  );
    ...
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        return sendError(
          ...
          500,
          "INTERNAL_ERROR",
          "An unexpected error occurred while fetching listing channel status.",
        );
      }
    },
  );
      ...
      schema: {
        ...
        response: {
          200: {
            type: "string",
            description:
              "ICS file download containing busy periods for external channels",
          },
        },
      },
    },
      ...
      try {
        ...
        function foldLine(line: string): string {
          ...
          while (offset < bytes.length) {
            ...
            chunks.push(
              (first ? "" : " ") +
                bytes.slice(offset, offset + chunkSize).toString("utf8"),
            );
            offset += chunkSize;
            first = false;
          }
        ...
        const listingName = (listing.name ?? "Kainook").replace(
          /[\\;,]/g,
          "\\$&",
        );
        ...
        reply
          ...
          .header(
            "Content-Disposition",
            `attachment; filename="listing-${id}.ics"`,
          )
        ...
        return sendError(
          ...
          500,
          "INTERNAL_ERROR",
          "An unexpected error occurred while exporting calendar.",
        );
      }
    },
  );
...
// ── Background polling ────────────────────────────────────────────────────────
// Feed polling is driven by the BullMQ IcalPoller dispatcher in jobs.ts,
// which enqueues one retryable IcalFeedSync job per due feed (see syncFeed
// and getDueIcalFeedIds above).
