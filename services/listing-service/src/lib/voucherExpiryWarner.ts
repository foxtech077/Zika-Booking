...
export function startVoucherExpiryWarner() {
  console.log("[Voucher Expiry Warner] Background job initialized.");
  // Check once every 4 hours (or configure via environment variable)
  const CHECK_INTERVAL_MS = 4 * 60 * 60 * 1000;
  // Warning window: Vouchers expiring in the next 24 hours
  ...
  // Initial startup trigger after 15 seconds
  setTimeout(() => {
    checkAndWarn().catch(() => null);
  }, 15000);
