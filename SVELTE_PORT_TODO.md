...
- [x] **A2. Saved payment methods** — FIXED. `getSavedPaymentMethods()` (`GET /guests/me/payment-methods`) is added to `payment-api.ts`, and the review payment step lists the guest's saved Stripe cards (default pre-selected, "Use a new card" option); a selected saved card is charged directly via `initiatePayment`, otherwise the card element flow runs (`apps/web-svelte/src/routes/(main)/booking/review/+page.svelte`).
- [x] **A3. Voucher wallet + tier-aware validation** — FIXED. The review page now passes `guestTier` (and country) to `/vouchers/validate`, and loads the guest's auto-assigned `/vouchers/wallet` into a dropdown to apply a wallet voucher (`apps/web-svelte/src/lib/booking-api.ts`, `apps/web-svelte/src/routes/(main)/booking/review/+page.svelte`). The promotion-override guard is covered by the existing promotion-vs-voucher discount selection.
- [x] **A4. Post-checkout "Ready to review" CTA** — FIXED. The booking review page stores the booking/listing review context at payment capture, and the listing detail page renders a "Ready to review" CTA (GiveReviewEntry equivalent) for that listing, linking to the reviews screen (`apps/web-svelte/src/routes/(main)/booking/review/+page.svelte`, `apps/web-svelte/src/routes/(main)/listings/[category]/[id]/+page.svelte`, `src/lib/review-context.ts`).
- [x] **A5. Cancellation policy label on booking detail** — FIXED. Both booking detail pages (`my-bookings/[id]` and `bookings/[code]`) now render a "Cancellation Policy" card using a shared `CANCELLATION_POLICY_LABEL` mapping, matching apps/web (`apps/web-svelte/src/lib/booking-utils.ts`).
...
- [x] **E1. Provider dashboard** — FIXED (core + rich management). A provider dashboard lives under `/dashboard` in web-svelte: `(provider)/+layout.svelte` with an auth guard and sidebar/mobile nav, plus pages for overview (`/provider/dashboard`), listings (list + status toggle + delete + blank-draft create + rich edit form with photo upload, amenity picker, room-type manager for hotels, geocoding, instant-booking toggle), availability & channels (monthly availability calendar + iCal feeds with sync/remove), bookings (filter + search), reviews (list + reply), earnings (12-month chart + totals), payments (overview + payout history + payout detail + booking revenue + Stripe Connect onboarding), messages (guest conversations), notifications, and settings (merchant profile). API client in `src/lib/provider-api.ts`. Document-upload presign/confirm/delete helpers are exposed but no document-management UI is wired yet.
