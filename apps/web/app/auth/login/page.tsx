...
export default function LoginPage() {
  ...
  const handleGoogleCredentialResponse = async (response: any) => {
    setError(null);
    // Google's own popup closes the moment an account is picked, so without
    // this the page sits inert while the credential is exchanged server-side.
    ...
    try {
      ...
      if (accessError) { setError(accessError); setGoogleLoading(false); return; }
      setSession(data.tokens.accessToken, data.user as any);
      // Deliberately stays true through the redirect — clearing it here would
      // flash the idle button for a frame before the route changes.
      ...
      <main className="flex min-h-screen min-h-[100dvh] flex-col lg:h-screen lg:overflow-y-auto">
        ...
        <div className="mx-auto my-auto w-full max-w-[600px] px-6 py-10 sm:px-8 lg:px-10 lg:py-14">
          ...
          <div className="relative mb-2.5 h-12 w-full">
            <div
              aria-hidden
              className={`pointer-events-none absolute inset-0 flex items-center justify-center gap-3 rounded-xl border border-gray-200 bg-white text-sm font-semibold text-gray-700 shadow-sm transition-opacity ${googleLoading ? "opacity-70" : ""}`}
            >
              {googleLoading ? (
                <>
            ...
            {/* pointer-events-none while in flight stops a second click reaching
                Google's invisible iframe and firing a duplicate exchange. */}
            <div
              ref={googleBtnRef}
              className={`google-btn-proxy absolute inset-0 overflow-hidden opacity-0 ${googleLoading ? "pointer-events-none" : ""}`}
            />
          </div>
