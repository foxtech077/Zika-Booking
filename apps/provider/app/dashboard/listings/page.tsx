...
export default function ListingsPage() {
  ...
  const handleCreate = (cat: ListingCategory) => {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {canSubmit(selected.status) && (
                <Button
                  variant="primary"
                  size="sm"
                  onClick={() => setConfirm({ action: "submit", listing: selected })}
                >
                  Submit for Review
                </Button>
              )}
              {canActivate(selected.status) && (
                <Button
                  variant="success"
                  size="sm"
                  onClick={() => setConfirm({ action: "activate", listing: selected })}
                >
                  Activate Listing
                </Button>
              )}
              {canDeactivate(selected.status) && (
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => setConfirm({ action: "deactivate", listing: selected })}
                >
                  Deactivate
                </Button>
              )}
              {canDelete(selected.status) && (
                <Button
                  variant="danger"
                  size="sm"
                  onClick={() => setConfirm({ action: "delete", listing: selected })}
                >
                  Delete Draft
                </Button>
              )}
      ...
      <ConfirmModal
        ...
        variant={confirm?.action === "delete" ? "danger" : confirm?.action === "deactivate" ? "danger" : "primary"}
        confirmLabel={
