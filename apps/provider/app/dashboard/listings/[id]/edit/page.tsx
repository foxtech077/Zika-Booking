...
import { MediaUploader } from "@/app/dashboard/listings/components/MediaUploader";
import { DocumentUploader } from "@/app/dashboard/listings/components/DocumentUploader";
...
export default function EditListingPage() {
  ...
  // Dynamic Wizard Steps Definition (Declared after listing query is declared)
  const STEPS = [
  ...
  // Client-Side Step Validation Helper
  const validateStep = (stepId: string): string[] => {
    ...
    if (stepId === "basic") {
      ...
      if (!fields.pricePerNight || Number(fields.pricePerNight) <= 0) {
        errors.push("Valid Price per Night / Daily Rental Price is required.");
      }
      if (!fields.currency) errors.push("Currency is required.");
      if (!fields.minStayNights || Number(fields.minStayNights) <= 0) {
        errors.push("Minimum stay duration must be a positive number.");
      }
    } else if (stepId === "specs") {
      if (listing?.category === "hotel") {
        if (!fields.unitCount || Number(fields.unitCount) <= 0) {
          errors.push("Total Inventory Unit Count must be greater than 0.");
        }
      } else if (listing?.category === "apartment") {
        if (!fields.maxGuests || Number(fields.maxGuests) <= 0) {
        ...
        if (!fields.carYear || Number(fields.carYear) < 1900 || Number(fields.carYear) > 2100) {
          errors.push("Valid Vehicle Year (1900-2100) is required.");
        }
        if (!fields.seats || Number(fields.seats) <= 0) {
          errors.push("Seats Capacity must be greater than 0.");
        }
      }
    }
    return errors;
  ...
  // ── Form States (bind to listing on load) ──────────────────────────────────
  const [fields, setFields] = useState<Partial<Listing>>({});
  const [selectedAmenities, setSelectedAmenities] = useState<string[]>([]);
  const [customAmenities, setCustomAmenities] = useState<string[]>([]);

  // Initialize form state once data loaded
  ...
  const addCustomAmenity = () => {
    ...
    if (customAmenities.includes(customAmenityInput.trim())) return;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-fade-in pb-16">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {listing.status === "draft" && (
            <span className="text-xs text-slate-400 font-medium">Draft Mode</span>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        {/* Navigation Tabs (Sequential Stepper Layout) */}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "border-primary bg-primary-50 text-primary-900 font-semibold"
        ...
        <div className="lg:col-span-3 space-y-6">
          <form onSubmit={handleSave}>
            <Card className="min-h-[420px]">
              ...
              {activeTab === "basic" && (
                <div className="space-y-4 animate-fade-in">
                  ...
                  <Textarea
                    ...
                    placeholder="Provide a detailed, attractive description of your accommodation or service details..."
                    rows={6}
                  />
                ...
                <div className="space-y-4 animate-fade-in">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Select
                      ...
                      error={attemptedSubmit && !fields.currency ? "Currency is required" : undefined}
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Input
                      ...
                      required
                      error={attemptedSubmit && (!fields.minStayNights || Number(fields.minStayNights) <= 0) ? "Minimum stay duration is required" : undefined}
                    />
                    <Select
              ...
              {activeTab === "amenities" && (
                <div className="space-y-5 animate-fade-in">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">Check standard inclusions or add custom tag tags.</p>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {customAmenities.map((tag) => (
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => removeCustomAmenity(tag)}
                ...
                <div className="space-y-4 animate-fade-in">
                  ...
                  {listing.category === "hotel" && (
                    <div className="space-y-4">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Determine room limits and room types.</p>
                  ...
                  {listing.category === "apartment" && (
                    <div className="space-y-4">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Provide sizing specifications and stay incentives.</p>
                      ...
                      <div className="border-t border-border pt-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <h4 className="text-sm font-semibold text-slate-800">Long-Stay Discounts (UC-3.3)</h4>
                        ...
                        {fields.longStayEnabled && (
                          <div className="grid grid-cols-3 gap-4 p-4 rounded-xl bg-slate-50 animate-fade-in">
                            <Input
                              label="Min Stays Required (Nights)"
                              type="number"
                              min="7"
                              value={fields.longStayMinNights ?? 30}
                              onChange={(e) => handleFieldChange("longStayMinNights", e.target.value)}
                            />
                            <Select
                              label="Discount Type"
                              value={fields.longStayDiscountType ?? "percentage"}
                              onChange={(e) => handleFieldChange("longStayDiscountType", e.target.value)}
                              options={[
                            ...
                            <Input
                              ...
                              type="number"
                              min="0"
                              value={fields.longStayDiscountValue ?? 0}
                              onChange={(e) => handleFieldChange("longStayDiscountValue", e.target.value)}
                            />
                          </div>
                        )}
                      </div>
                  ...
                  {listing.category === "car" && (
                    <div className="space-y-4">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Car rental details, transmission, and mileage limits.</p>
                      
                      <div className="grid grid-cols-3 gap-4">
                        <Input
                          label="Vehicle Make"
                          value={fields.carMake ?? ""}
                          onChange={(e) => handleFieldChange("carMake", e.target.value)}
                          placeholder="E.g., Tesla"
                          required
                          error={attemptedSubmit && !fields.carMake?.trim() ? "Vehicle make is required" : undefined}
                        />
                        <Input
                          label="Vehicle Model"
                          ...
                          required
                          error={attemptedSubmit && !fields.carModel?.trim() ? "Vehicle model is required" : undefined}
                        />
                        <Input
                          label="Year"
                          type="number"
                          min="1900"
                          max="2100"
                          value={fields.carYear ?? 2024}
                          onChange={(e) => handleFieldChange("carYear", e.target.value)}
                          required
                          error={attemptedSubmit && (!fields.carYear || Number(fields.carYear) < 1900 || Number(fields.carYear) > 2100) ? "Valid vehicle year (1900-2100) is required" : undefined}
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <Select
                          label="Transmission"
                          value={fields.transmission ?? "automatic"}
                        ...
                        <Select
                          ...
                          value={fields.fuelType ?? "electric"}
                          onChange={(e) => handleFieldChange("fuelType", e.target.value)}
                          options={[
                            { value: "petrol", label: "Gasoline / Petrol" },
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <Input
                          ...
                          type="number"
                          min="1"
                          value={fields.seats ?? 5}
                          onChange={(e) => handleFieldChange("seats", e.target.value)}
                          required
                          error={attemptedSubmit && (!fields.seats || Number(fields.seats) <= 0) ? "Seats capacity is required" : undefined}
                        />
                        <Input
                          label="Doors"
                          type="number"
                          min="2"
                          value={fields.doors ?? 4}
                      ...
                      <div className="border-t border-border pt-4 grid grid-cols-2 gap-4">
                        <Select
                          ...
                          value={fields.mileagePolicy ?? "unlimited"}
                          onChange={(e) => handleFieldChange("mileagePolicy", e.target.value)}
                          options={[
                        ...
                        {fields.mileagePolicy === "limited" && (
                          <Input
                            ...
                            type="number"
                            min="10"
                            value={fields.mileageLimitKm ?? 300}
                            onChange={(e) => handleFieldChange("mileageLimitKm", e.target.value)}
                          />
                        )}
                      </div>
                    </div>
              ...
              {/* Tab 5: Media Upload & verification */}
              {activeTab === "media" && (
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mt-6 pt-6 border-t border-border">
              ...
              <div className="flex items-center gap-2">
                ...
                {activeTab !== "media" ? (
                  ...
                  <>
                    {/* Activation Actions on final step */}
