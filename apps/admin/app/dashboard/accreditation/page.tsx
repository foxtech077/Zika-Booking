...
export default function AccreditationPage() {
  ...
  const params = Object.fromEntries(
    Object.entries({
      page: String(page),
      limit: "20",
      ...(isCountryManager && userCountryScope.length > 0 ? { country: userCountryScope[0] } : {}),
    }).filter(([, v]) => v !== "")
  );
  ...
  const tasks = isCountryManager && userCountryScope.length > 0
    ? rawTasks.filter((t) => {
        const country = t.listing?.country;
        return country && userCountryScope.some((sc) => sc.toLowerCase() === country.toLowerCase());
      })
    : rawTasks;
